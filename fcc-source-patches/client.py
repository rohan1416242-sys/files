...
# MAX-TPS PATCH: reasoning_effort is now sent as a top-level field by the
# _apply_nim_reasoning postprocessor in request_options.py. The profile's
# reasoning encoder is NO_REASONING (no-op) — the actual encoding happens
# in the postprocessor chain.
...
class NvidiaNimProvider(OpenAIChatProvider):
    ...
    def _provider_failure_override(self, error: Exception) -> ExecutionFailure | None:
        """Classify NVIDIA-specific 400/403/429 responses by their actual semantics."""
        # MAX-TPS PATCH: NIM returns 403 "Authorization failed" as a rolling
        # rate-limit/quota signal (not a true permission denial). Treat it as
        # retryable so FCC backs off and retries instead of failing hard.
        if isinstance(error, openai.PermissionDeniedError):
            bodies = _nim_error_bodies(error)
            if any(_is_nim_quota_403(body) for body in bodies):
                # Rolling rate-limit: retryable, but with a long backoff.
                return overloaded_provider_failure()
            return None

        # MAX-TPS PATCH: NIM sometimes returns 429 as a generic APIError
        # instead of RateLimitError. Normalize it so the retry path catches it.
...
def _is_nim_quota_403(body: Mapping[str, Any]) -> bool:
    """Detect NIM's rolling rate-limit 403 ('Authorization failed').

    NIM returns HTTP 403 with body {'detail': 'Authorization failed'} when
    the API key has hit a rolling throughput quota. Unlike a true permission
    denial, this recovers after a cool-down and should be retried.
    """
    detail = body.get("detail")
    if not isinstance(detail, str):
        return False
    ...
    return (
        "authorization failed" in detail_lower
