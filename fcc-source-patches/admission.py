...
# MAX-TPS PATCH: bumped from 5 attempts / 2.0s base / 60s max to give NIM
# 429 rate-limit retries a real chance to recover before failing the user's
# request. Tuned from burn-test data: NIM's rolling rate-limit recovers in
# 1-5 minutes, so 10 attempts with 1->2->4->8->16->32->60->60->60->60s
# backoff covers most recovery windows while still failing fast on real errors.
UPSTREAM_TRANSIENT_TOTAL_ATTEMPTS = 10
DEFAULT_UPSTREAM_BASE_DELAY = 1.0
DEFAULT_UPSTREAM_MAX_DELAY = 120.0
