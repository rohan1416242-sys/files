...
def build_nim_request_body(
    ...
    # MAX-TPS PATCH: include the reasoning encoder in the postprocessor chain
    # so reasoning_effort=max is sent as a top-level field (required by
    # thinkingmachines/inkling on NIM). The encoder is defined inline here to
    # avoid a circular import with client.py.
    return build_openai_chat_request_body(
        request_data,
        reasoning=reasoning,
        policy=NIM_REQUEST_POLICY,
        postprocessors=(
            _apply_nim_reasoning,
...
def _apply_nim_reasoning(
    body: dict[str, Any], _request: MessagesRequest, policy: ReasoningPolicy
) -> None:
    """MAX-TPS PATCH: encode reasoning_effort as a top-level field for NIM.

    NIM (and thinkingmachines/inkling specifically) accepts reasoning_effort
    as a top-level parameter: 'low', 'medium', 'high', 'max', or 'none'.
    """
    ...
    if policy.effort is not None:
        mapped = effort_map.get(policy.effort.value)
