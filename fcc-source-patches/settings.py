...
class Settings(BaseSettings):
    ...
    # MAX-TPS PATCH: default to thinkingmachines/inkling (highest TPS in tests).
    model: str = "nvidia_nim/thinkingmachines/inkling"
    ...
    # MAX-TPS PATCH: lowered defaults so FCC paces NIM more gently and
    # triggers fewer upstream 429s. Override per-install with env vars if needed.
    #   Old: 40 req / 60s, 5 concurrent
    #   New: 15 req / 60s, 2 concurrent (NIM-friendly)
    provider_rate_limit: int = Field(default=15, validation_alias="PROVIDER_RATE_LIMIT")
    ...
    # MAX-TPS PATCH: read timeout default is 0 (None) so long-running
    # reasoning models (thinkingmachines/inkling, etc.) never get killed
    # mid-stream. Set HTTP_READ_TIMEOUT=120 to restore the old default.
    http_read_timeout: float = Field(
        default=0.0, validation_alias="HTTP_READ_TIMEOUT"
    )
    http_write_timeout: float = Field(
        default=30.0, validation_alias="HTTP_WRITE_TIMEOUT"
    )
